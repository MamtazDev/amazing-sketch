            ...
            <Navbar expand="lg">
                <Container>
                    ...
                    <Navbar.Collapse id="basic-navbar-nav">
                        <Nav className="me-auto align-items-center ms-4">
                            {navItems.map((item, index) => (
                                <li key={index} className='nav-items'>
                                    <Nav.Link href={item.link} className='fw-semibold ff-poppins text-capitalize'>
                                        {item.name}
                                    </Nav.Link>
                                </li>
                            ))}
                        ...
                        <Nav className='align-items-center'>
                            {
                                userNav.map((navItem, index) => (
                                    <li key={index} className='nav-items'>
                                        <Nav.Link href={navItem.link} className='fw-semibold ff-poppins text-capitalize'>
                                            {navItem.name === 'avatar' ? (
                                                <span className='d-flex gap-4 justify-content-center align-items-center'>
                                                    <span>
                                                        <svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <rect width="11" height="11" rx="5.5" transform="matrix(-1 0 0 1 11 0)" fill="#26CECE" />
                                                        </svg>
                                                    </span>
